...
class Model(GObject.GObject):
    ...
    __gsignals__ = {
        'box-changed': (GObject.SIGNAL_RUN_FIRST, GObject.TYPE_NONE,
                        ([object])),
        'box-zpos-changed': (GObject.SIGNAL_RUN_FIRST, GObject.TYPE_NONE,
                             ([object])),
        'box-added': (GObject.SIGNAL_RUN_FIRST, GObject.TYPE_NONE, ([object])),
        'box-removed': (GObject.SIGNAL_RUN_FIRST, GObject.TYPE_NONE,
                        ([object])),
        'page-rendered': (GObject.SIGNAL_RUN_FIRST, GObject.TYPE_NONE,
                          ([object])),
        'box-rendered': (GObject.SIGNAL_RUN_FIRST, GObject.TYPE_NONE,
                         ([object])),
    ...
    def __init__(self, pdffile=None, loadfile=None):
        ...
        self._box_render_process = GProcess(
            target=self._box_render_proc, childcb=self._box_rendered_wakeup)
    ...
    def get_rendered_box_or_queue(self, box, scale, x_offset, y_offset,
        ...
        try:
            ...
            result, _scale, page, x, y, width, height, dscale, _x_offset, _y_offset, uploaded = self._rendered_boxes[
                box]
            ...
            if similar_surface and not uploaded:
                ...
                surf = similar_surface.create_similar(
                    cairo.CONTENT_COLOR_ALPHA, iwidth, iheight)
                ...
                result = surf
                self._rendered_boxes[box] = (result, _scale, page, x, y, width,
                                             height, dscale, _x_offset,
                                             _y_offset, True)
    ...
    def get_rendered_page_or_queue(self, page, scale, x_offset, y_offset,
                                   similar_surface):
        try:
            result, _page, _scale, _x_offset, _y_offset, uploaded = self._rendered_pages[
                page]

            # Check whether surface can and is not uploaded to the X server
            if similar_surface and not uploaded:
                iwidth, iheight = result.get_width(), result.get_height()
                surf = similar_surface.create_similar(cairo.CONTENT_COLOR,
                                                      iwidth, iheight)
                cr = cairo.Context(surf)
                cr.set_operator(cairo.OPERATOR_SOURCE)
                cr.set_source_surface(result, 0, 0)
                ...
                result = surf
                self._rendered_pages[page] = (result, _page, _scale, _x_offset,
                                              _y_offset, True)

            if scale != _scale or x_offset != _x_offset or y_offset != _y_offset:
                # Queue a render at the correct scale
                self._queue_page_render_at_scale(page, scale, x_offset,
                                                 y_offset)
    ...
    def _real_emit_tif(self, filename, progress_cb, *pbargs):
        ...
        def write_tif(surface, tmpdir, page):
            ...
            os.spawnv(os.P_WAIT, '/usr/bin/convert', [
                'convert', png, '-units', 'PixelsPerInch', '-density', '300',
                '-monochrome', '-compress', 'Group4', tif
            ])
            os.unlink(png)
        ...
        progress = 0
        GObject.idle_add(self._emit_progress_cb, progress_cb, progress,
                         len(self._boxes), *pbargs)
        ...
        input = [
            os.path.join(tmpdir, "%i.tif" % i) for i in range(1, page + 1)
        ]
        ...
        # done ...
        GObject.idle_add(self._emit_progress_cb, progress_cb,
                         len(self._boxes) + 1, len(self._boxes) + 1, *pbargs)
    ...
    def move_box_up(self, box):
        index = self._boxes.index(box)
        if index + 1 < len(self._boxes) and self._boxes[index +
                                                        1].dpage == box.dpage:
            _ = self._boxes[index + 1]
            self._boxes[index + 1] = self._boxes[index]
            self._boxes[index] = _
    ...
    def save_to_file(self, filename):
        ...
        for b in self._boxes:
            f.write("%f %f %f %f %f %f %f %i %i\n" %
                    (b.sx, b.sy, b.width, b.height, b.dx, b.dy, b.dscale,
                     b.spage, b.dpage))
    ...
    def _load_from_file(self):
        ...
        if not os.path.isabs(self.pdffile):
            self.pdffile = os.path.join(
                os.path.dirname(os.path.abspath(self.loadfile)), self.pdffile)
    ...
    def _queue_box_render_at_scale(self, box, scale, x_offset, y_offset):
        render_info = (scale, box.spage, box.sx, box.sy, box.width, box.height,
                       box.dscale, x_offset, y_offset)
    ...
    def _render_box(self, data):
        ...
        try:
            surface = cairo.ImageSurface(cairo.FORMAT_ARGB32,
                                         int(math.ceil(scaled_width)),
                                         int(math.ceil(scaled_height)))
        except MemoryError:
            sys.stderr.write(
                "Cannot render box at this zoom, not enough memory!\n")
            return None
    ...
    def _render_page(self, data):
        ...
        try:
            ...
            sys.stderr.write(
                "Cannot render page at this zoom, not enough memory!\n")
            return None
