...
class Page(GooCanvas.CanvasItemSimple, GooCanvas.CanvasItem):
    ...
    def do_paint(self, cr, bounds, scale):
        ...
        result = self._model.get_rendered_page_or_queue(
            self._page, scale, x_offset, y_offset, cr.get_target())
        if result is None:
            ...
            cr.translate(-width * scale / 2.0 - extends[0] * scale,
                         -height * scale / 2.0 - extends[1] * scale)
        ...
        if self._drag_active:
            ...
            cr.rectangle(x + lw / 2, y + lw / 2, width - lw, height - lw)
            cr.stroke()
            cr.restore()
...
class PDFView(GooCanvas.Canvas):
    ...
    def set_model(self, model):
        ...
        self._model = model
        self._page_rendered_id = self._model.connect("page-rendered",
                                                     self._page_rendered_cb)
        ...
        for i in range(self._model.document.get_n_pages()):
            page = Page(
                self._model, i, x, y, fill_color="black", parent=self._root)
