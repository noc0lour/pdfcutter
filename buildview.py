...
class Box(GooCanvas.CanvasItemSimple, GooCanvas.CanvasItem):
    ...
    def do_button_press_event(self, target, event):
        if event.button == 1:
            ...
            if not self.has_focus and not self._canvas._item_in_focus_group(
                    self):
    ...
    def do_key_press_event(self, target, event):
        ...
        if keyname == 'minus':
            self._canvas._model.move_boxes_down(
                [item._box for item in self._canvas._iter_focused_items()])
            handled = True
        elif keyname == 'plus':
            self._canvas._model.move_boxes_up(
                [item._box for item in self._canvas._iter_focused_items()])
            handled = True
    ...
    def do_paint(self, cr, bounds, scale):
        ...
        result = self._canvas._model.get_rendered_box_or_queue(
            self._box, rscale, x_offset, y_offset, cr.get_target())
        if result is None:
            ...
            cr.translate(-width * nscale / 2.0 - extends[0] * nscale,
                         -height * nscale / 2.0 - extends[1] * nscale)
            ...
            cr.scale(1 / iscale * self._box.dscale, 1 / iscale *
                     self._box.dscale)
        ...
        if self.get_canvas().outlines:
            ...
            cr.rectangle(self.x + lw / 2, self.y + lw / 2, self.width - lw,
                         self.height - lw)
            if not self.has_focus and not self._canvas._item_in_focus_group(
                    self):
    ...
    def do_paint(self, cr, bounds, scale):
        ...
        if self.get_canvas().grid:
            ...
            for x in range(1, int(self.width / 72 * 2.54) + 1):
                cr.move_to((int(self.x * scale + x / 2.54 * 72 * scale) + 0.5)
                           / scale, 0)
                cr.line_to((int(self.x * scale + x / 2.54 * 72 * scale) + 0.5)
                           / scale, self.height)
            ...
            for y in range(1, int(self.height / 72 * 2.54) + 1):
                cr.move_to(0, (
                    int(self.y * scale + y / 2.54 * 72 * scale) + 0.5) / scale)
                cr.line_to(self.width, (
                    int(self.y * scale + y / 2.54 * 72 * scale) + 0.5) / scale)
                cr.set_line_width(1 / scale)
                cr.set_dash([4 / scale, 4 / scale])
...
class BuildView(GooCanvas.Canvas):
    ...
    def set_model(self, model):
        ...
        self._model = model
        self._page_rendered_id = self._model.connect("box-rendered",
                                                     self._box_rendered_cb)
        self._box_changed_id = self._model.connect("box-changed",
                                                   self._box_changed_cb)
        self._box_zpos_changed_id = self._model.connect(
            "box-zpos-changed", self._box_zpos_changed_cb)
        self._box_added_id = self._model.connect("box-added",
                                                 self._box_added_cb)
        self._box_removed_id = self._model.connect("box-removed",
                                                   self._box_removed_cb)
        self._header_text_changed_id = self._model.connect(
            "header-text-changed", self._header_text_changed_cb)
    ...
    def _add_page(self):
        ...
        self.set_bounds(
            0, 0, pwidth + 2 * _PADDING,
            math.ceil(pheight + _PADDING) * len(self._pages) + _PADDING * 3)
    ...
    grid = GObject.property(
        type=bool, default=True, setter=set_grid, getter=get_grid)
    outlines = GObject.property(
        type=bool, default=True, setter=set_outlines, getter=get_outlines)
