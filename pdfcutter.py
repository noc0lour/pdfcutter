...
class MainWindow(object):
    ...
    def autosave(self):
        ...
        if self._model.loadfile is None:
            message_id = statusbar.push(
                context_id, "Could not autosave, please save the project!")
        else:
            try:
                self._model.save_to_file(self._model.loadfile)
                message_id = statusbar.push(context_id,
                                            "Autosaved the project.")
            except:
                message_id = statusbar.push(
                    context_id,
                    "Error saving the project! Maybe try to save somewhere else."
                )
    ...
    def export_pdf(self, *args):
        ...
        if result == Gtk.ResponseType.OK:
            ...
            self._model.main_iter_emit_pdf(
                filename, self.export_pdf_pb_updater, dialog, pbar)
    ...
    def export_png(self, *args):
        ...
        if result == Gtk.ResponseType.OK:
            ...
            dialog = Gtk.Dialog(
                title="Creating monochrome TIF File %s" % filename,
                parent=self._window,
                flags=Gtk.DialogFlags.MODAL |
                Gtk.DialogFlags.DESTROY_WITH_PARENT)
            pbar = Gtk.ProgressBar()
            ...
            self._model.main_iter_emit_tif(
                filename, self.export_pdf_pb_updater, dialog, pbar)
    ...
    def save_to_file_with_error_dialog(self, filename):
        try:
            ...
            msg = Gtk.MessageDialog(
                parent=self._window, type=Gtk.MessageType.WARNING)
            ...
            msg.format_secondary_markup(
                "Maybe try to store the file somewhere else?")
            msg.run()
            msg.destroy()
    ...
    def show_about_dialog(self, *args):
        ...
        about_dialog.set_comments(
            "Britzel Cut, ein PDF zuschneide tool für die Fachschaft Elektro- und Informationstechnik Universität Karlsruhe"
        )
        about_dialog.set_program_name("Britzel Cut")
        about_dialog.set_website(
            "https://fachschaft.etec.uni-karlsruhe.de/trac/fs_etec/")
        about_dialog.run()
        about_dialog.destroy()

    def quit_application(self, *args):
        buttons = (Gtk.STOCK_CLOSE, Gtk.ResponseType.OK, Gtk.STOCK_CANCEL,
        ...
        dialog.format_secondary_text(
            "Sind Sie sicher, dass Sie Britzel Cut beenden wollen?")
